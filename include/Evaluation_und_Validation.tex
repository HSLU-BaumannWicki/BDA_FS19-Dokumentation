...
\label{ch:Eval}
In diesem Kapitel wird evaluiert, ob das Projekt und dessen produzierte Artefakte die Anforderungen erfüllen. Dazu wurden alle Anforderungen nochmals zusammengetragen und jede einzelne mit dem jeweiligen Artefakt überprüft. Das Resultat wurde schliesslich in Form einer einfach lesbaren Tabelle festgehalten. Aus diesem Ist/Soll-Vergleich konnte schliesslich festgestellt werden, dass alle priorisierten Anforderungen erfüllt werden konnten. Daher kann dieses Projekt als erfolgreich betrachtet werden.
...
Die Verwendung von UnitTests, in Zusammenarbeit mit der CI/CD-Pipeline, führte zu einer besseren Verifizierung der Codequalität und Funktionalität. Zudem konnten dank der vorhanden Tests Refactorings durchgeführt werden, ohne das Vertrauen in die Applikation zu verlieren. Diese erwähnten Vorteile machten es möglich, dass auf Anpassungen vor Ort einfacher reagiert werden konnte. Durch diesen Mehrwert erachten wir den Einsatz von UnitTest als sehr erfolgreich.
